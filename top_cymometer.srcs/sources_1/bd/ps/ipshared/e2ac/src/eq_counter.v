...
//
//   f_test = f_s * (EQ_TEST + tdc_rise/NUM_TAPS - tdc_fall/NUM_TAPS) / EQ_STAND
//
...
module eq_counter #(
    ...
    //
    // The synchronizer depth is tied to the pipeline latency of tdc.v by
    // T = L + 1, where L = TDC_LATENCY. Derivation:
    //   bit_sync(STAGES=T) outputs gate_fx(n) = gate(n-(T-1)).
    ...
    //   interval (m-1-T, m-T], i.e. delay chain sample S_(m-T).
    //   By the tdc.v contract the result of S_(m-T) appears at
    //   tdc_value(m-T+L), and cycle m reads tdc_value(m-1). Equating the two
    //   gives T = L + 1 -- no extra delay needed on the TDC side.
    //
    // T tracked L through 3/2, then 4/3, and now 6/5 as the chain grew to 256
    // taps. Changing either depth requires redoing this derivation.
    //========================================================================
    wire gate_fx;
    bit_sync #(.STAGES(6)) u_gate_sync (
    ...
    always @(posedge clk_fx) begin
        if (!rst_fx_n) begin
            ...
            if (gate_fx_rise) begin
                test_cnt    <= 32'd0;
                tdc_rise_fx <= tdc_vld ? tdc_val : 8'd0;
            ...
            if (gate_fx_fall) begin
                ...
                tdc_fall_fx <= tdc_vld ? tdc_val : 8'd0;
            end
        end
    end
