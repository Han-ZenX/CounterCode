...
//   [31:26] tdc     TDC phase value 0..63
...
module ts_engine #(
    ...
    reg [31:0] free_run_cnt_d1, free_run_cnt_d2;
    ...
    // Per the tdc.v contract, the result of S_m appears at tdc_value(m+2),
    // so this transition's TDC value appears at tdc_value(n+1).
    //
    // If capture happens on the m-th rising edge, an always block reads the
    // (m-1) values of every signal:
    //   to read tdc_value(n+1)      ->  m - 1 = n + 1  ->  m = n + 2
    //   to read edge_raw(n)         ->  delay edge_raw by two cycles to get
    //                                   edge_d2, since
    //                                   edge_d2(m-1) = edge_raw(m-3) = edge_raw(n)
    //   the transition belongs to period n-1, so the coarse counter must read
    //                                   free_run_cnt_d2(m-1) = (m-1)-2 = n-1
    //
    // Conclusion: on the cycle where edge_d2 is asserted, the triple
    // {free_run_cnt_d2, tdc_value} describes exactly the same rising edge.
    // Changing the pipeline depth of any element requires redoing this.
    //========================================================================
    wire [5:0] tdc_value;
    wire       tdc_valid;
    ...
    wire edge_hit = edge_d2 & ts_en_fs;
    wire do_capture = edge_hit & (skip_cnt == 32'd0);
    ...
    always @(posedge clk_fs) begin
        if (!rst_fs_n)
            ...
            skip_cnt <= (skip_cnt == 32'd0) ? skip_limit : (skip_cnt - 1'b1);
    end
    ...
    // Classic criterion: wrap bits differ while the low bits match, meaning
    // the write pointer has lapped the read pointer exactly once.
    // Synchronizer latency makes rd_ptr_bin_at_wr slightly stale, so the full
    // detection errs conservative -- which is the safe direction.
    wire [FIFO_ADDR_WIDTH:0] rd_ptr_bin_at_wr;
    wire fifo_full = (wr_ptr_bin[FIFO_ADDR_WIDTH] != rd_ptr_bin_at_wr[FIFO_ADDR_WIDTH]) &&
                     (wr_ptr_bin[FIFO_ADDR_WIDTH-1:0] == rd_ptr_bin_at_wr[FIFO_ADDR_WIDTH-1:0]);

    reg [23:0] seq_cnt;        // entry sequence number
    ...
    // when fitting.
    wire [63:0] ts_word = {free_run_cnt_d2,   // [63:32] coarse count
                           tdc_value,          // [31:26] TDC phase
                           ovf_mark,           // [25]    overflow mark
                           tdc_valid,          // [24]    TDC fine value valid
                           seq_cnt};           // [23:0]  sequence number
