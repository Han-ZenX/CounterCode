...
module ts_engine #(
    ...
    // Per the tdc.v contract, the result of S_m appears at tdc_value(m+3),
    // so this transition's TDC value appears at tdc_value(n+2).
    //
    // If capture happens on the m-th rising edge, an always block reads the
    // (m-1) values of every signal:
    //   to read tdc_value(n+2)      ->  m - 1 = n + 2  ->  m = n + 3
    //   to read edge_raw(n)         ->  edge_dk(m-1) = edge_raw(m-1-k), and
    //                                   m-1-k = n with m = n+3 gives k = 2,
    //                                   so edge_d2 is still the right tap
    //   the transition belongs to period n-1, so the coarse counter must read
    //                                   free_run_cnt_dX(m-1) = (m-1)-X = n-1
    //                                   with m = n+3 this gives X = 3
    //
    // Conclusion: on the cycle where edge_d2 is asserted, the pair
    // {free_run_cnt_d3, tdc_value} describes exactly the same rising edge.
    //
    // Note the asymmetry: TDC_LATENCY went from 2 to 3, which moved the
    // coarse counter from d2 to d3 but left edge_d2 unchanged -- the capture
    // cycle and the TDC output both shifted by one, so their difference held.
    // Do not assume the two delays track each other; rederive.
    //========================================================================
    wire [5:0] tdc_value;
    wire       tdc_valid;
    ...
    // when fitting.
    wire [63:0] ts_word = {free_run_cnt_d3,   // [63:32] coarse count
