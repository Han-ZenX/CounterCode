...
module eq_counter #(
    ...
    //
    // A 4-stage synchronizer is used so it lines up exactly with the 3-cycle
    // pipeline latency of tdc.v. Derivation:
    //   bit_sync(STAGES=4) outputs gate_fx(n) = gate(n-3).
    ...
    //   interval (m-5, m-4], i.e. delay chain sample S_(m-4).
    //   By the tdc.v contract the result of S_(m-4) appears at tdc_value(m-1),
    //   and cycle m reads exactly tdc_value(m-1) -- no extra delay needed.
    //
    // This was 3 stages while TDC_LATENCY was 2. Splitting the TDC adder tree
    // to meet timing pushed the latency to 3, so this had to grow by one to
    // keep the gate phase and the TDC reading describing the same edge.
    // Changing either depth requires redoing this derivation.
    //========================================================================
    wire gate_fx;
    bit_sync #(.STAGES(4)) u_gate_sync (
