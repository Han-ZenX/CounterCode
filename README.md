...
f(t) 曲线的打印由 `STARTUP:TRACE` 控制，**上电默认关闭**。它是调试手段而非输出 ——
结果本来就走 SCPI 返回，而 64 行曲线在 115200 下约 320 ms，会直接计入每次测量的节拍。
调试某颗器件时发 `STARTUP:TRACE 64` 打开，产线上保持 0。配置、首个可检出边沿时刻、
频率分辨率与最终结果**始终打印**，共约 7 行，是出问题时唯一的线索。
