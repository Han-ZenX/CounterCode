...
`FREQ:GATE:TIME` 和 `START:GATE:TIME` 这两条命令，**设置时参数是秒，查询时返回值是毫秒**：

```
FREQ:GATE:TIME 0.1      → 内部 GATE_TIME = 100（毫秒）
FREQ:GATE:TIME?         → 返回 "100.000"
```

这是重构前 TCP 侧就有的行为（`atof(args) * 1000`），本次原样保留，未作修改。
如果要统一单位，需要同时改动上位机，属于协议变更。
